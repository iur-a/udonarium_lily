...
import { SkyWayConnection as SkyWay2023Connection } from './skyway2023/skyway-connection';

let _backendMode: string = '';
...
export class Network {
  ...
  configure(config: { backend?: { mode?: string, url?: string }, webrtc?: { key?: string } }): void {
    if (config?.backend?.mode) _backendMode = config.backend.mode;
    if (config?.backend?.url) _backendUrl = config.backend.url;
    if (config?.webrtc?.key) this.key = config.webrtc.key;
  }
  ...
  private initializeConnection(): Connection {
    ...
    if (_backendMode === 'skyway2023') {
      store = new SkyWay2023Connection();
      (store as any).setBackendUrl(_backendUrl);
    } else {
      store = new SkyWayConnection();
