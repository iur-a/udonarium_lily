...
Opal.queue(function(Opal) {/* Generated by Opal 1.7.4 */
  var $module = Opal.module, $klass = Opal.klass, $const_set = Opal.const_set, $eqeqeq = Opal.eqeqeq, $def = Opal.def, $hash2 = Opal.hash2, $truthy = Opal.truthy, $rb_gt = Opal.rb_gt, $rb_divide = Opal.rb_divide, $eqeq = Opal.eqeq, $rb_plus = Opal.rb_plus, $rb_times = Opal.rb_times, self = Opal.top, $nesting = [], nil = Opal.nil, $$$ = Opal.$$$;
  ...
  return (function($base, $parent_nesting) {
    ...
    return (function($base, $parent_nesting) {
      ...
      return (function($base, $super, $parent_nesting) {
        var self = $klass($base, $super, 'Siren');

        var $nesting = [self].concat($parent_nesting), $$ = Opal.$r($nesting), $proto = self.$$prototype;

        $proto.round_type = $proto.randomizer = nil;
        
        $const_set($nesting[0], 'ID', "Siren");
        $const_set($nesting[0], 'NAME', "終末アイドル育成TRPG セイレーン");
        $const_set($nesting[0], 'SORT_KEY', "せいれえん");
        $const_set($nesting[0], 'HELP_MESSAGE', "・判定: SL+a<=b±c\n" + "  a=達成値への修正(0の場合は省略)\n" + "  b=能力値\n" + "  c=判定への修正(0の場合は省略、複数可)\n" + "例)判定修正-10の装備を装着しながら【技術：60】〈兵器：2〉で判定する場合。\n" + "SL+2<=60+40-10\n" + "\n" + "・育成: TR$a<=b\n" + "  a=育成した回数\n" + "  b=ヘルス\n" + "例）ヘルスの現在値が60で2回目の【身体】の育成を行う場合。\n" + "TR$2<=60\n");
        
        $def(self, '$eval_game_system_specific_command', function $$eval_game_system_specific_command(command) {
          var self = this, $ret_or_1 = nil;

          if ($eqeqeq(/^SL/, ($ret_or_1 = command))) {
            return self.$check_action(command)
          } else if ($eqeqeq(/^TR/, $ret_or_1)) {
            return self.$check_training(command)
          } else {
            return nil
          }
        });
        ...
        $def(self, '$check_action', function $$check_action(command) {
          var self = this, parser = nil, parsed = nil, target = nil, dice = nil, dig10 = nil, dig1 = nil, achievement_value = nil;

          
          parser = $$$($$('Command'), 'Parser').$new("SL", $hash2(["round_type"], {"round_type": self.round_type})).$restrict_cmp_op_to("<=");
          ...
          if ($truthy($rb_gt(dice, target))) {
            return $$('Result').$failure("(1D100<=" + (target) + ") ＞ " + (dice) + " ＞ 失敗")
          };
          dig10 = $rb_divide(dice, 10).$floor();
          dig1 = dice['$%'](10);
          if ($eqeq(dig10, 0)) {
            dig10 = 10
          };
          ...
          achievement_value = $rb_plus($rb_plus(dig10, dig1), parsed.$modify_number());
          return $$('Result').$success("(1D100<=" + (target) + ") ＞ " + (dice) + " ＞ 成功(達成値：" + (achievement_value) + ")");
        });
        
        $def(self, '$check_training', function $$check_training(command) {
          var self = this, parser = nil, parsed = nil, count = nil, target = nil, dice = nil, dig10 = nil, dig1 = nil, achievement_value = nil;

          
          parser = $$$($$('Command'), 'Parser').$new("TR", $hash2(["round_type"], {"round_type": self.round_type})).$restrict_cmp_op_to("<=").$enable_dollar().$disable_modifier();
          parsed = parser.$parse(command);
          if ($truthy(parsed['$nil?']())) {
            return nil
          };
          ...
          achievement_value = $rb_plus(dig10, dig1);
          if ($truthy($rb_gt(dice, target))) {
            return $$('Result').$failure("(1D100<=" + (target) + ") ＞ " + (dice) + " ＞ 失敗(能力値減少：10 / ヘルス減少：" + (achievement_value) + ")")
          };
          return $$('Result').$success("(1D100<=" + (target) + ") ＞ " + (dice) + " ＞ 成功(能力値上昇：" + ($rb_plus($rb_times(count, 5), achievement_value)) + " / ヘルス減少：" + (achievement_value) + ")");
        });
        return self.$register_prefix("SL", "TR");
