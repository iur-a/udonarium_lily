...
Opal.queue(function(Opal) {/* Generated by Opal 1.7.4 */
  var $module = Opal.module, $klass = Opal.klass, $const_set = Opal.const_set, $truthy = Opal.truthy, $def = Opal.def, $eqeq = Opal.eqeq, $rb_plus = Opal.rb_plus, $rb_ge = Opal.rb_ge, $thrower = Opal.thrower, $hash2 = Opal.hash2, self = Opal.top, $nesting = [], nil = Opal.nil, $$$ = Opal.$$$;
  ...
  return (function($base, $parent_nesting) {
    ...
    return (function($base, $parent_nesting) {
      ...
      return (function($base, $super, $parent_nesting) {
        ...
        $const_set($nesting[0], 'ID', "NeverCloud");
        $const_set($nesting[0], 'NAME', "ネバークラウドTRPG");
        $const_set($nesting[0], 'SORT_KEY', "ねはあくらうとTRPG");
        $const_set($nesting[0], 'HELP_MESSAGE', "・LIST　のコマンドを入力して、ルールの解説・2D6表、一覧の表示\n" + "\n" + "・判定(xNC±y>=z)\n" + "　xD6の判定を行います。ファンブル、クリティカルの場合、その旨を出力します。\n" + "　x：振るダイスの数。\n" + "　±y：固定値・修正値。省略可能。\n" + "　z：目標値。省略可能。\n" + "　ダイスの出目ふたつが6ならクリティカル(自動成功)\n" + "　ダイスの出目すべてが1ならファンブル(自動失敗)\n" + "　例）　2NC+2>=5　1NC\n");
        ...
        $def(self, '$eval_game_system_specific_command', function $$eval_game_system_specific_command(command) {
          ...
          } else if ($truthy($$('TEXTS')['$key?'](command))) {
            return $$('TEXTS')['$[]'](command).$chomp()
          } else if ($truthy($$('TABLES')['$key?'](command))) {
            return self.$roll_tables(command, $$('TABLES'))
          } else if ($truthy($$('ALIAS_TEXTS')['$key?'](command))) {
        ...
        $def(self, '$check_action', function $$check_action(command) {try { var $t_eval_return = $thrower('eval_return'); 
          var $a, self = this, m = nil, dice_count = nil, modify_str = nil, modify_number = nil, cmp_str = nil, target = nil, dice_list = nil, dice_value = nil, dice_str = nil, total = nil, result = nil, sequence = nil;
          ...
          cmp_str = m['$[]'](3);
          target = ($a = m['$[]'](4), ($a === nil || $a == null) ? nil : $a.$to_i());
          if ($eqeq(modify_number, 0)) {
            modify_str = ""
          };
          ...
          result = ($eqeq(dice_list.$count(1), dice_count) ? (((total = 0), "ファンブル")) : ($truthy($rb_ge(dice_list.$count(6), 2)) ? ("クリティカル") : ($truthy(target) ? (($truthy($rb_ge(total, target)) ? ("成功") : ("失敗"))) : nil)));
