...
Opal.queue(function(Opal) {/* Generated by Opal 1.7.4 */
  var $module = Opal.module, $klass = Opal.klass, $const_set = Opal.const_set, $truthy = Opal.truthy, $def = Opal.def, $eqeqeq = Opal.eqeqeq, $send = Opal.send, $eqeq = Opal.eqeq, $rb_plus = Opal.rb_plus, $rb_le = Opal.rb_le, $rb_ge = Opal.rb_ge, $rb_minus = Opal.rb_minus, $range = Opal.range, $nesting = [], nil = Opal.nil;
  ...
  return (function($base, $parent_nesting) {
    ...
    return (function($base, $parent_nesting) {
      ...
      return (function($base, $super, $parent_nesting) {
        ...
        $const_set($nesting[0], 'ID', "Paradiso");
        $const_set($nesting[0], 'NAME', "チェレステ色のパラディーゾ");
        $const_set($nesting[0], 'SORT_KEY', "ちえれすていろのはらていいそ");
        $const_set($nesting[0], 'HELP_MESSAGE', "◆判定　(nCPt[f]@c)、(nD20<=t[f]@c)　n:ダイス数（省略時:1）　t:目標値（省略時:14）　f:絶不調の追加ファンブル値　c:人機一体の追加クリティカル値\n" + "　例）CP12　CP(13+1)　3CP12[18,19]@7\n" + "◆各種表\n" + "　・ラジオ・マリエッタ表　RMT\n" + "　・移動表　TOT\n" + "　・探索表　EXT\n" + "　・補給表　SUT\n" + "◆ダメージチェック　(DCa[20,30])　a:【攻撃力】、[20]:20mm機銃追加、[30]:30mmガンポッド追加\n" + "　例）DC4:【攻撃力】4でダメージチェック　DC5[20]:【攻撃力】5でダメージチェック、うち1つは20mm機銃　DC5[20,30]:【攻撃力】5でダメージチェック、うち1つは20mm機銃、うち1つは30mmガンポッド\n");
        self.$register_prefix("\\d*D20<=", "\\d*CP", "RMT", "TOT", "EXT", "SUT", "DC");
        ...
        $def(self, '$eval_game_system_specific_command', function $$eval_game_system_specific_command(command) {
          ...
          switch (command.$upcase().valueOf()) {
            case "RMT":
              result = self.$get_radiomarietta_table()
              break;
            case "TOT":
              result = self.$get_takeoff_table()
              break;
            case "EXT":
              result = self.$get_exploration_table()
              break;
            case "SUT":
              result = self.$get_flightsupply_table()
              break;
            default:
              nil
          };
        ...
        $def(self, '$getJudgeResult', function $$getJudgeResult(command) {
          var self = this, $ret_or_1 = nil, number = nil, $ret_or_2 = nil, target = nil, fumble1 = nil, fumble2 = nil, critical = nil, dice = nil, dicetext = nil, crit_flg = nil, fumb_flg = nil, succ_flg = nil, result = nil, text = nil;

          
          if ($eqeqeq(/^(\d+)?D20<=(\d+)?(\[(\d+)(,(\d+))?\])?(@(\d+))?$/i, ($ret_or_1 = command))) {
            
            number = ($truthy(($ret_or_2 = $$('Regexp').$last_match(1))) ? ($ret_or_2) : (1)).$to_i();
            target = ($truthy(($ret_or_2 = $$('Regexp').$last_match(2))) ? ($ret_or_2) : (14)).$to_i();
            fumble1 = ($truthy(($ret_or_2 = $$('Regexp').$last_match(4))) ? ($ret_or_2) : (21)).$to_i();
            fumble2 = ($truthy(($ret_or_2 = $$('Regexp').$last_match(6))) ? ($ret_or_2) : (21)).$to_i();
            critical = ($truthy(($ret_or_2 = $$('Regexp').$last_match(8))) ? ($ret_or_2) : (21)).$to_i();
          } else if ($eqeqeq(/^(\d+)?CP(\d+)?(\[(\d+)(,(\d+))?\])?(@(\d+))?$/i, $ret_or_1)) {
          ...
          fumb_flg = false;
          succ_flg = false;
          $send(number, 'times', [], function $$1(){var self = $$1.$$s == null ? this : $$1.$$s;
          ...
          if ($eqeq(crit_flg, true)) {
            result = "クリティカル"
          } else if ($eqeq(fumb_flg, true)) {
            result = "ファンブル"
          } else if ($eqeq(succ_flg, true)) {
            result = "成功"
          } else {
            result = "失敗"
          };
        ...
        $def(self, '$getDamageResult', function $$getDamageResult(command) {
          var self = this, biggun = nil, $ret_or_1 = nil, attack = nil, $ret_or_2 = nil, dice = nil, dicetext = nil, damage = nil, doubledam = nil, tripledam = nil, result = nil, text = nil;

          
          biggun = [0, 0, 0];
          if ($eqeqeq(/^DC(\d+)(\[(\d+)(,(\d+))?(,(\d+))?(,(\d+))?(,(\d+))?(,(\d+))?\])?$/i, ($ret_or_1 = command))) {
            
            attack = ($truthy(($ret_or_2 = $$('Regexp').$last_match(1))) ? ($ret_or_2) : (1)).$to_i();
            biggun['$[]='](0, ($truthy(($ret_or_2 = $$('Regexp').$last_match(3))) ? ($ret_or_2) : (0)).$to_i());
            biggun['$[]='](1, ($truthy(($ret_or_2 = $$('Regexp').$last_match(5))) ? ($ret_or_2) : (0)).$to_i());
            biggun['$[]='](2, ($truthy(($ret_or_2 = $$('Regexp').$last_match(7))) ? ($ret_or_2) : (0)).$to_i());
            biggun['$[]='](3, ($truthy(($ret_or_2 = $$('Regexp').$last_match(9))) ? ($ret_or_2) : (0)).$to_i());
            biggun['$[]='](4, ($truthy(($ret_or_2 = $$('Regexp').$last_match(11))) ? ($ret_or_2) : (0)).$to_i());
            biggun['$[]='](5, ($truthy(($ret_or_2 = $$('Regexp').$last_match(13))) ? ($ret_or_2) : (0)).$to_i());
          } else {
            return nil
          };
          dice = 0;
          ...
          doubledam = 0;
          tripledam = 0;
          $send(biggun, 'each', [], function $$2(bg){
            
            if (bg == null) bg = nil;
            
            switch (bg.valueOf()) {
              case 30:
                return (tripledam = $rb_plus(tripledam, 1))
              case 20:
                return (doubledam = $rb_plus(doubledam, 1))
              default:
                return nil
            };});
          $send(attack, 'times', [], function $$3(){var $a, self = $$3.$$s == null ? this : $$3.$$s;
            if (self.randomizer == null) self.randomizer = nil;
            ...
            if ($truthy($rb_ge(tripledam, 1))) {
              ...
              damage['$[]=']($rb_minus(dice, 1), $rb_plus(damage['$[]']($rb_minus(dice, 1)), 3));
              tripledam = $rb_plus(tripledam, -1);
              return (dicetext = $rb_plus(dicetext, "【30mm】"));
            } else if ($truthy($rb_ge(doubledam, 1))) {
              
              damage['$[]=']($rb_minus(dice, 1), $rb_plus(damage['$[]']($rb_minus(dice, 1)), 2));
              doubledam = $rb_plus(doubledam, -1);
              return (dicetext = $rb_plus(dicetext, "【20mm】"));
            } else {
              return ($a = [$rb_minus(dice, 1), $rb_plus(damage['$[]']($rb_minus(dice, 1)), 1)], $send(damage, '[]=', $a), $a[$a.length - 1])
            };}, {$$s: self});
        ...
        $def(self, '$get_radiomarietta_table', function $$get_radiomarietta_table() {
          var self = this, dice = nil, $ret_or_1 = nil, text = nil;

          
          dice = self.randomizer.$roll_once(20);
          if ($eqeqeq(1, ($ret_or_1 = dice))) {
            text = "「なんてこった！　ここで事故のお知らせだ！」\n通行止め……。ランダムなマス1つを決定する。この一日中、そのマスに移動する事はできない。"
          } else if ($eqeqeq($range(2, 4, false), $ret_or_1)) {
            text = "「今日はまたずいぶんと湿気てるねぇ……。古傷がある人は要注意だよ」\n天候が悪い。この一日中、「●移動」のアクションで移動できるマス数は常に1マス低くなる。"
          } else if ($eqeqeq($range(5, 10, false), $ret_or_1)) {
            text = "「日常は至上！　異常は退場！　なんにもないからラジオは以上！　……なんつて」\nいつもどおりの日々。のんびりとした風で、何事もなし。"
          } else if (($eqeqeq(11, $ret_or_1) || ($eqeqeq(12, $ret_or_1)))) {
            text = "「それじゃ、本日のメインコーナー。行ってみよう！」\n軽妙なトーク。PC全員の【乗り手コンディション】が1点小さくなる。"
          } else if ($eqeqeq($range(13, 15, false), $ret_or_1)) {
        ...
        $def(self, '$get_takeoff_table', function $$get_takeoff_table() {
          var self = this, dice = nil, $ret_or_1 = nil, text = nil;
          ...
          } else if ($eqeqeq($range(10, 12, false), $ret_or_1)) {
        ...
        $def(self, '$get_exploration_table', function $$get_exploration_table() {
          var self = this, dice = nil, $ret_or_1 = nil, text = nil;
          ...
          } else if ($eqeqeq($range(5, 8, false), $ret_or_1)) {
          ...
          } else if ($eqeqeq($range(10, 13, false), $ret_or_1)) {
        ...
        return $def(self, '$get_flightsupply_table', function $$get_flightsupply_table() {
          var self = this, dice = nil, $ret_or_1 = nil, text = nil;
          ...
          } else if ($eqeqeq($range(7, 9, false), $ret_or_1)) {
