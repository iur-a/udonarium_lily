...
Opal.queue(function(Opal) {/* Generated by Opal 1.7.4 */
  var $module = Opal.module, $klass = Opal.klass, $const_set = Opal.const_set, $truthy = Opal.truthy, $def = Opal.def, $hash2 = Opal.hash2, $rb_plus = Opal.rb_plus, $rb_ge = Opal.rb_ge, $rb_le = Opal.rb_le, $send = Opal.send, $hash = Opal.hash, $range = Opal.range, self = Opal.top, $nesting = [], nil = Opal.nil, $$$ = Opal.$$$;
  ...
  return (function($base, $parent_nesting) {
    ...
    return (function($base, $parent_nesting) {
      ...
      return (function($base, $super, $parent_nesting) {
        var self = $klass($base, $super, 'VisionConnect');

        var $nesting = [self].concat($parent_nesting), $$ = Opal.$r($nesting), $proto = self.$$prototype;

        $proto.randomizer = nil;
        
        $const_set($nesting[0], 'ID', "VisionConnect");
        $const_set($nesting[0], 'NAME', "ヴィジョンコネクト");
        $const_set($nesting[0], 'SORT_KEY', "ういしよんこねくと");
        $const_set($nesting[0], 'HELP_MESSAGE', "・判定(VC+x@c#f>=y)\n" + "  !：コマンドの最初に付けると致命的失敗が全てアクシデントになる。\n" + "  x：修正値。能力値、戦闘値、その他修正値など。省略可。\n" + "  y：目標値。省略時は決定的成功/致命的失敗のみ表示。\n" + "  c：クリティカル値。@ごと省略可。省略時は12。\n" + "  f：ファンブル値。#ごと省略可。省略時は3。\n" + "  (例)VC+3>=8\n" + "      VC+7@11>=12\n" + "      !VC+6-1#4>=10\n" + "\n" + "・各種表\n" + "  アクシデント表 AT\n" + "  トラブル表 TT\n");
        ...
        $def(self, '$check_action', function $$check_action(command) {
          var self = this, parser = nil, parsed = nil, stamina_zero = nil, critical_target = nil, $ret_or_1 = nil, fumble_target = nil, accident_target = nil, dice_arr = nil, dice_sum = nil, result_sum = nil, is_critical = nil, is_fumble = nil, is_accident = nil, is_trouble = nil, is_success = nil, result_str = nil, sequence = nil;

          
          parser = $$$($$('Command'), 'Parser').$new(/!?VC/, $hash2(["round_type"], {"round_type": $$$($$('RoundType'), 'FLOOR')})).$enable_critical().$enable_fumble().$restrict_cmp_op_to(nil, ">=");
          parsed = parser.$parse(command);
          if ($truthy(parsed['$nil?']())) {
            return nil
          };
          stamina_zero = parsed.$command()['$[]'](0)['$==']("!");
          critical_target = ($truthy(($ret_or_1 = parsed.$critical())) ? ($ret_or_1) : (12));
          fumble_target = ($truthy(($ret_or_1 = parsed.$fumble())) ? ($ret_or_1) : (3));
          accident_target = ($truthy(stamina_zero) ? (fumble_target) : (2));
          dice_arr = self.randomizer.$roll_barabara(2, 6);
          dice_sum = dice_arr.$sum();
          result_sum = $rb_plus(dice_sum, parsed.$modify_number());
          is_critical = $rb_ge(dice_sum, critical_target);
          is_fumble = $rb_le(dice_sum, fumble_target);
          is_accident = $rb_le(dice_sum, accident_target);
          is_trouble = ($truthy(($ret_or_1 = is_fumble)) ? (is_accident['$!']()) : ($ret_or_1));
          if ($truthy(is_critical)) {
            
            is_success = true;
            result_str = "決定的成功";
          } else if ($truthy(is_accident)) {
            
            is_success = false;
            result_str = "致命的失敗(アクシデント)";
          } else if ($truthy(is_trouble)) {
            
            is_success = false;
            result_str = "致命的失敗(トラブル)";
          } else if ($truthy(parsed.$target_number()['$nil?']())) {
            is_success = nil
          } else if ($truthy($rb_ge(result_sum, parsed.$target_number()))) {
          ...
          sequence = ["(" + (parsed.$to_s("after_modify_number")) + ")", "" + (dice_sum) + "[" + (dice_arr.$join(",")) + "]" + ($$('Format').$modifier(parsed.$modify_number())), result_sum, result_str].$compact();
          return $send($$('Result').$new(), 'tap', [], function $$1(r){var $a;
            ...
            r['$fumble='](is_fumble);
            r['$success='](($truthy(($ret_or_1 = is_success)) && ($ret_or_1)));
            return ($a = [($truthy(is_success['$nil?']()) ? (false) : (is_success['$!']()))], $send(r, 'failure=', $a), $a[$a.length - 1]);});
        });
        $const_set($nesting[0], 'TABLES', $hash2(["AT", "TT"], {"AT": $$$($$('DiceTable'), 'Table').$new("アクシデント表", "1D6", ["頭がぼんやりして、まぶたが重くなってきた……。これは睡魔の襲来？　キャラクターの操作がおぼつかなくなる。シーン終了まで能力値判定、戦闘値判定の達成値に-3される。スタミナを3点消費することで、この効果を打ち消すことができる。", "キーボード、マウス、ゲームパッドなどが操作不能になった！　キャラクターを操作することができない。戦闘中の場合は次のラウンドの準備プロセス終了までキャラクターアクションを行うことができず、スキルや特技の使用もできない。スタミナを3点消費することで、この効果を打ち消すことができる。", "急に画面が真っ暗に！　パソコンやゲーム機を見ると、動作はしている。これはモニタの問題かっ！　キャラクターを操作することができない。戦闘中の場合は次のラウンドの終了までキャラクターアクションを行うことができず、スキル、特技の使用もできない。スタミナを4点消費することで、この効果を打ち消すことができる。", "突然、通信回線が不調となり、切断されてしまった！　急いで再ログインしなければ！　シーンから自動的に退場となる。戦闘中の場合は次のラウンドの準備プロセス終了後、登場できる。スタミナを4点消費することで、この効果を打ち消すことができる。", "いきなり画面が真っ黒になり、パソコンやゲーム機が再起動し始めた……。シーンから自動的に退場となる。戦闘中の場合は次のラウンドの終了後、登場できる。スタミナを5点消費することで、この効果を打ち消すことができる。", "突然、画面が消えた。いや、画面だけじゃない。電化製品がすべて止まっているようだ。もしや、これは停電！？　シーンから自動的に退場となる。次のシーンの開始時に登場できる。スタミナを5点消費することで、この効果を打ち消すことができる。"]), "TT": $$$($$('DiceTable'), 'D66RangeTable').$new("トラブル表", $hash($range(11, 13, false), "チャットで誤爆(発言ミス)をしてしまった。恥ずかしさで、スタミナが1点減少する。", $range(14, 16, false), "かまってほしいのか、ペットがちょっとした悪戯をしてきた。ごめん、今は忙しいのだ。ペットを取得していない場合は何も起こらない。ペットを取得していた場合、罪悪感によりスタミナが1点減少する。", $range(21, 23, false), "何かの用事があるのか、それとも食事の時間なのか、家族から声を掛けられた。家族を取得していない場合は何も起こらない。家族を取得していた場合、気が焦ってスタミナが2点減少する。", $range(24, 26, false), "玄関のチャイムが鳴り、「宅配便でーす」の声が外から聞こえてきた。こんな時にっ！？　家族がいれば、荷物を受け取ってもらえるのだが……。家族を取得している場合は何も起こらない。家族を取得していない場合、スタミナが2点減少する。", $range(31, 33, false), "操作中に勢い余って腕が飲み物に当たってしまい、中身がこぼれてしまった。あとで掃除しないと……。ドリンクを取得していない、あるいはすべて使用済みである場合は何も起こらない。ドリンクを1個失う。", $range(34, 36, false), "キーボードやゲームパッドの調子があまりよくない。やっぱり、ゲーミングデバイスに買い換えた方がいいか……。デバイスを取得している場合は何も起こらない。デバイスを取得していない場合、ストレスによりスタミナが2点減少する。", $range(41, 43, false), "知り合いから電話が掛かってきた。電話しながらの操作はちょっと大変だ。より集中しなければならないため、スタミナが2点減少する。", $range(44, 46, false), "急にお手洗いに行きたくなってきた。ちょっと我慢しなければならないため、スタミナが2点減少する。", $range(51, 51, false), "レアモンスターがポップ(出現)したとチャットで通知が来た！　でも、今は行くことができない……。ブレイブを取得していない場合は何も起こらない。ブレイブを取得している場合、悔しさでスタミナが3点減少する。", $range(52, 52, false), "出品しているアイテムのマーケットでの相場が下がったと知り合いからチャットが飛んできた。マイスターを取得していない場合は何も起こらない。マイスターを取得している場合、悲しさでスタミナが3点減少する。", $range(53, 53, false), "操作の方法が分からなくなって、焦りまくる。ノービスを取得していない場合は何も起こらない。ノービスを取得している場合、混乱でスタミナが3点減少する。", $range(54, 54, false), "ギルドのメンバーからギルドを抜けたいという相談のチャットが飛んできた。リーダーを取得していない場合は何も起こらない。リーダーを取得している場合、驚きのあまりスタミナが3点減少する。", $range(55, 55, false), "知り合いから攻略の手伝いを頼むチャットが飛んできた。ごめんなさい、今はちょっと無理……。ヘルパーを取得していない場合は何も起こらない。ヘルパーを取得している場合、申し訳なさでスタミナが3点減少する。", $range(56, 56, false), "つきまとってくるユーザーから、しつこくチャットが飛んでくる。面倒くさいなぁ。フェイバリットを取得していない場合は何も起こらない。フェイバリットを取得している場合、煩わしさでスタミナが3点減少する。", $range(61, 61, false), "誰かと一緒にプレイするのに慣れていないためか、ちょっと緊張しているかもしれない。ローンウルフを取得していない場合は何も起こらない。ローンウルフを取得している場合、緊張でスタミナが3点減少する。", $range(62, 62, false), "事前に入手していた情報が間違っていた！？　どう対応してよいか分からず、焦りまくる。ブレインを取得していない場合は何も起こらない。ブレインを取得している場合、焦りのあまりスタミナが3点減少する。", $range(63, 63, false), "配信でトラブルが発生！？　対応に慌ててしまう。ストリーマーを取得していない場合は何も起こらない。ストリーマーを取得している場合、狼狽によってスタミナが3点減少する。", $range(64, 64, false), "使っているゲーミングデバイスの調子がよくない。ガジェッターを取得していない場合は何も起こらない。ガジェッターを取得している場合、いらだちでスタミナが3点減少する。", $range(65, 65, false), "合間にプレイしている別のゲームや流し見していた動画に注意が向いて、操作をミスしてしまう。カジュアルを取得していない場合は何も起こらない。カジュアルを取得している場合、後悔でスタミナが3点減少する。", $range(66, 66, false), "ハードコアを取得していない場合は何も起こらない。トラブル表の51～65の項目の効果を受ける。ハードコア以外に取得しているスタイルに合わせて、効果を適用すること(たとえば、ブレイブならば51、マイスターなら52となる)。"))}).$freeze());
        return self.$register_prefix("!?VC", $$('TABLES').$keys());
